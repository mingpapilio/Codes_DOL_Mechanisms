...
for i=1:length(essen)
    ...
    for j=1:length(size)
        ...
        cstar=1-N*((1-e)+(2*e-1)*qopt-e*(qopt^N))/((N-2)*e+1);
        if isreal(cstar) && cstar>0 && isreal(qopt) %makes sure area where 
            %DOL not favoured doesnt mess up results!
        results(end+1,:)=[e N qopt cn cstar];
        else
        results(end+1,:)=[e N 0 1 .001];
        end
