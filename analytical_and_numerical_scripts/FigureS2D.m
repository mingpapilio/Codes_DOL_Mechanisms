...
%fecundity is accelerating in the proportion of helpers in the group. 
...
alpha=(1.5); %shape of return from proportion of helpers (decelerating when alpha<1)
...
for i=1:length(size)
    for j=1:length(essen)
        ...
        stochoptfit=results(results(:,2)==N&results(:,1)==e,5);
        matt(i,j)=stochoptfit/coordfit;
    end
end
