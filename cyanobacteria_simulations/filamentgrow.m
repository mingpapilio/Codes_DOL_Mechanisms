...
function res = filamentgrow18(start, params, deets)
...
for t=1:trials
    ...
    for n=1:(L-4)
        ...
        res(t,:)=[timelog(end) length(reps) mean(goodflux(reps)) (1/(timelog(end)))*sum(growthrate(reps)) var(goodflux(reps)) sum(Gs.*(1-Gs)) mean(growthrate(reps)) var(growthrate(reps)) mean(inter) var(inter)];
